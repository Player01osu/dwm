...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (ulineall || m->tagset[m->seltags] & 1 << i) /* if there are conflicts, just move these lines directly underneath both 'drw_setscheme' and 'drw_text' :) */
			drw_rect(drw, x + ulinepad, bh - ulinestroke - (user_bh), w - (ulinepad * 2), ulinestroke, 1, 0);
		if (occ & 1 << i)
			drw_rect(drw, x + ulinepad, bh - ulinestroke - (user_bh - 1.5), w - (ulinepad * 2), ulinestroke, 1, 0);
		x += w;
	}
